...
from view_synthesis.utils import prepare_device, is_main_process, prepare_experiment, mse2psnr, get_minibatches, log_losses
...
def train(rank: int, cfg: CfgNode) -> None:
    ...
    for iteration in range(start_iter, total_load_iterations):
        ...
        for j, (ro, rd, target_object_ids, target_pixels) in enumerate(zip(ro_minibatches, rd_minibatches, tgt_object_ids_minibatches, tgt_pixel_minibatches)):
            ...
            if cfg.is_distributed:
                shape_params, texture_params = models['embedding'].module.get_params_tensor()
            else:
                shape_params, texture_params = models['embedding'].get_params_tensor()
            ...
            if is_main_process(cfg.is_distributed) and i > 0:
                if i % cfg.experiment.print_every == 0:
                    ...
                    log_string = log_losses(writer, "train", i, iteration, time.time()-then, losses_dict, scheduler.get_last_lr()[0])
...
def validate(cfg: CfgNode,
        ...
        if is_main_process(cfg.is_distributed):
            if val_iter > 0 and val_iter % cfg.experiment.val_print_every == 0:
                losses_dict = {"nerf_loss_coarse": nerf_loss_coarse.item(),
                               ...
                               "total_loss": loss.item(),
                               "psnr": psnr}
                log_string = log_losses(writer, "val-optim", val_iter, iteration, time.time()-val_then, losses_dict)
    ...
    if is_main_process(cfg.is_distributed):
        ...
        log_string = log_losses(writer, "val", val_iter, iteration, time.time()-render_then, render_losses_dict)
